...
public class AuthController {
    ...
    public ResponseEntity<?> verify(@Valid @RequestBody VerifyRequest request) {
        ...
        if (user.isVerified()) return ResponseEntity.badRequest().body(Map.of("error", "Already verified"));
        ...
        return ResponseEntity.ok(Map.of("token", token, "email", user.getEmail(), "fullName", user.getFullName(), "message", "Email verified successfully"));
    }
    ...
    public ResponseEntity<?> forgotPassword(@RequestBody Map<String, String> request) {
        User user = userRepository.findByEmail(request.get("email")).orElse(null);
        if (user == null) return ResponseEntity.badRequest().body(Map.of("error", "No account found with that email"));
        String code = String.format("%06d", new Random().nextInt(999999));
        user.setVerificationCode(code);
        userRepository.save(user);
        return ResponseEntity.ok(Map.of("email", user.getEmail(), "verificationCode", code, "message", "Reset code generated"));
    }
    ...
    public ResponseEntity<?> resetPassword(@RequestBody Map<String, String> request) {
        User user = userRepository.findByEmail(request.get("email")).orElse(null);
        if (user == null) return ResponseEntity.badRequest().body(Map.of("error", "User not found"));
        if (!user.getVerificationCode().equals(request.get("code")))
